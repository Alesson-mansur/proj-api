...
RSpec.describe 'Task API' do
	...
	describe 'GET /tasks' do
		before do
			create_list(:task, 5, user_id: user.id)#cria uma lista de objetos definidos
			#pela mesma factory
	...
	describe 'POST /tasks' do
		...
		context 'when params are valid' do
			...
			it 'saves task innto database' do
			...
			it 'assigns created task to the current user' do
		...
		context 'When params are invalid' do
			...
			it 'does not save task in database' do
	...
	describe 'PUT /tasks/:id' do
		...
		context 'when params are valid' do
			...
			it 'retuns json for updated task' do
				expect(json_body[:title]).to eq(task_params[:title])
			end
		...
		context 'when params are invalid' do
			...
			it 'retuns json error for title' do
				expect(json_body[:errors]).to have_key(:title)
			end
